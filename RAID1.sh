...
clear
echo "RAID1: The following parameters have been loaded:"
numHDD="$1"
echo "HDDs = " "$numHDD"
seekTime="$2"
echo "Seek = " "$seekTime"
...
HDDtoWrite=1
clear
echo "RAID1 SIM START" >> "$fileName".out
for var in "$@"
do
		...
		#Output and write to disk twice, one for each mirror.
		for i in $(seq 1 $numHDD)
		do
...
echo "RAID1 SIM END" >> "$fileName".out
