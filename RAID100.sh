...
clear
echo "RAID100: The following parameters have been loaded:"
numHDD="$1"
echo "HDDs = " "$numHDD"
seekTime="$2"
echo "Seek = " "$seekTime"
...
if [ $numHDD -ge 8 ]
	then
		...
		HDDtoWrite=1
		clear
		echo "RAID100 SIM START" >> "$fileName".out
		for var in "$@"
		do
			#Mirror across 2 disks for every entry.
			for i in $(seq 1 2)
			do
	...
	echo "RAID100 SIM END" >> "$fileName".out
